...
imgRGB <- data.frame(
  x = rep(1:imgDm[2], each = imgDm[1]),#315个1，315个2.。。估计是列坐标/横坐标（第一个数字列坐标值）
...
plotOriginal <- ggplot(data = imgRGB) + #aes在ggplot中用(在geom_point里用也可以)，告诉ggplot它x坐标和y坐标的列表
...
kMeans <- kmeans(imgRGB[,c("R", "G", "B")], centers = k) #c()前面有没有逗号也是一样的。kmeans是二维数组类，将数据集分成k类，返回一个k个mean值，和一个分类的数据
